...
{
    use ThrottlesLogins, HasApiTokens, Notifiable;
    /**
    ...
    public function login(Request $request)
    {
        ...
        if (!User::where($this->username(), input($this->username()))
            ...
            return $this->sendFailedLoginResponse($request,
                'Моля активирайте вашия акаунт преди да се опитвате да влезете.');
        }
        // try to login
    ...
    private function sendLoginResponse(Request $request)
    {
        ...
        if ($user->getAttribute('logged_in_at')) {
            notify()->info('Info',
                'Последно сте влезнали на ' . $user->logged_in_at->diffForHumans(),
                'clock-o rotateIn animated', 8000);
        }
        else {
            notify()->info('Здравейте',
                'Здравейте ' . $user->fullname . '. Здравейте в ' . config('app.name'),
        ...
        log_activity('Login', $user->fullname . ' се вписа.');
