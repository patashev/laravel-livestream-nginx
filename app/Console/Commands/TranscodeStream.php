...
class TranscodeStream extends Command
{
    ...
    protected function setCommand($withFacebook, $slug)
    {
        ...
        $command = "/home/adm1n/ffmpeg/ffmpeg -i 'rtmp://127.0.0.1/live/${slug}' -async 1 -vsync -1 ";
        ...
        $command = $command . "${facebookrtmp} -c copy -f flv 'rtmp://127.0.0.1/show/${slug}_src' 2>/home/adm1n/HLS/live/${slug}-ffmpeg.log";
        $this->line($command);
    ...
    public function handle()
    {
        ...
        if($this->option('stop'))
        {
            ...
            //     '--stop' => true
            // ]);
            exec("rm /home/adm1n/HLS/live/${slug}.running");
            ...
            $files = explode("\n", shell_exec("find /home/adm1n/HLS/live/ -name 'index.m3u8' | grep ${slug}"));
        ...
        {
            exec("touch /home/adm1n/HLS/live/${slug}.running");
            ...
            $files = explode("\n", shell_exec("find /home/adm1n/HLS/live/ -name '*.ts' | grep ${slug}"));
            foreach($files as $file)
            {
                if(!empty($file))
                {
